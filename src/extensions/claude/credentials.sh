...
case "$(uname -s)" in
    Darwin)
        # macOS: Try keychain for Claude Max OAuth tokens
        # Claude stores tokens under service "claude.ai"

        # Try to get access token
        ACCESS_TOKEN=$(security find-generic-password -s "claude.ai" -a "access_token" -w 2>/dev/null || true)

        if [ -n "$ACCESS_TOKEN" ]; then
            # Check if token is expired and needs refresh
            EXPIRES_AT=$(security find-generic-password -s "claude.ai" -a "expires_at" -w 2>/dev/null || true)
            REFRESH_TOKEN=$(security find-generic-password -s "claude.ai" -a "refresh_token" -w 2>/dev/null || true)
            ...
            if [ -n "$EXPIRES_AT" ] && [ -n "$REFRESH_TOKEN" ] && [ "$NOW" -gt "$((EXPIRES_AT - 300))" ]; then
                ...
                if echo "$RESPONSE" | grep -q '"access_token"'; then
                    ...
                    if [ -n "$NEW_TOKEN" ]; then
                        ...
                        security add-generic-password -U -s "claude.ai" -a "access_token" -w "$ACCESS_TOKEN" 2>/dev/null || true

                        if [ -n "$NEW_EXPIRES" ]; then
                            NEW_EXPIRES_AT=$((NOW + NEW_EXPIRES))
                            security add-generic-password -U -s "claude.ai" -a "expires_at" -w "$NEW_EXPIRES_AT" 2>/dev/null || true
                        fi

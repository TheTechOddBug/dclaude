...
func HandleConfigCommand(args []string) {
	...
	case "get":
		if len(args) < 2 {
			fmt.Println("Usage: addt config get <key>")
			os.Exit(1)
		}
		getConfig(args[1])
	case "set":
		if len(args) < 3 {
			fmt.Println("Usage: addt config set <key> <value>")
			os.Exit(1)
		}
		setConfig(args[1], args[2])
	case "unset":
		if len(args) < 2 {
			fmt.Println("Usage: addt config unset <key>")
			os.Exit(1)
		}
		unsetConfig(args[1])
	case "path":
...
func printConfigHelp() {
	...
	fmt.Println("  list          List all configuration values")
	...
	fmt.Println("  1. Environment variables (e.g., ADDT_CPUS)")
